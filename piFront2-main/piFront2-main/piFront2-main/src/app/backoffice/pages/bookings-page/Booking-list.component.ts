...
export class BookingListComponent implements OnInit {
  ...
  cancelBooking(id?: number): void {
    ...
    Swal.fire({
      ...
      this.bookingService.cancel(id, 'STUDENT', 'Cancelled by student').subscribe(() => {
        this.loadBookings();
        Swal.fire({ icon: 'success', title: 'Cancelled', timer: 1800, showConfirmButton: false });
      });
    });
  }
  ...
  rejectBooking(id?: number): void {
    ...
    (Swal as any).fire({
      ...
      if (!result.isConfirmed) return;
      this.bookingService.reject(id, result.value || '').subscribe(() => {
        this.loadBookings();
        Swal.fire({ icon: 'success', title: 'Rejected', timer: 1800, showConfirmButton: false });
      });
    });
  }
