...
export class BookingFormComponent implements OnInit {
  ...
  onSubmit(): void {
    if (!this.booking.tutorId) {
      Swal.fire({ icon: 'warning', title: 'Missing Field', text: 'Please select a tutor', confirmButtonColor: '#2563eb' });
      return;
    }
    if (!this.booking.sessionDate) {
      Swal.fire({ icon: 'warning', title: 'Missing Field', text: 'Please select a date', confirmButtonColor: '#2563eb' });
      return;
    }
    if (!this.booking.startTime || !this.booking.endTime) {
      Swal.fire({ icon: 'warning', title: 'Missing Field', text: 'Please select a time slot', confirmButtonColor: '#2563eb' });
      return;
    }
    ...
    action.subscribe({
      next: () => {
        ...
        Swal.fire({ icon: 'success', title: 'Success!', text: 'Booking saved successfully.', timer: 2000, showConfirmButton: false })
          .then(() => this.router.navigate(['/admin/bookings']));
      },
      error: (err) => {
        this.isLoading = false;
        Swal.fire({ icon: 'error', title: 'Error', text: err.error?.message || 'An error occurred. Please try again.', confirmButtonColor: '#2563eb' });
      }
    });
  }
}
