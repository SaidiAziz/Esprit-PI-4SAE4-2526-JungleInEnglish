    ...
    this.frontBookingService.getTutorById(tutorId).subscribe({
      next: (tutor) => {
        this.tutor = tutor ? {
          ...
          bio: tutor.bio || 'Experienced English tutor passionate about helping students reach their goals.',
          rating: tutor.rating || 4.5,
          reviewCount: tutor.reviewCount || 0,
          experienceYears: tutor.experienceYears || 1,
          specialization: tutor.specialization?.length ? tutor.specialization : ['General English'],
          languages: tutor.languages?.length ? tutor.languages : ['English'],
          profilePicture: tutor.profilePicture || ''
        } : null;
        if (this.tutor && this.booking) {
          const duration = this.calcDuration(this.booking.startTime, this.booking.endTime);
          this.amount = (this.tutor.hourlyRate ?? 25) * duration;
        }
        this.loading = false;
