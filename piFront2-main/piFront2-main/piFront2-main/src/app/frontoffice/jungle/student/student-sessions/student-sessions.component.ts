    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.bookingService.endSession(sessionId).subscribe({
          ...
          error: (error: any) => Swal.fire({ icon: 'error', title: 'Erreur', text: 'Une erreur est survenue en terminant la session.' })
        });
      }
    });
  }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.feedbackService.delete(feedbackId).subscribe({
          ...
          error: (error: any) => Swal.fire({ icon: 'error', title: 'Erreur', text: 'Impossible de supprimer le feedback.' })
        });
      }
    });
  }
