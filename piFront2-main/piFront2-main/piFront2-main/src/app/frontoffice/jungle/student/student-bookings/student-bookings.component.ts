    ...
    Swal.fire({
      ...
      this.bookingService.cancel(bookingId, 'STUDENT', 'Cancelled by student').subscribe({
        next: () => {
          ...
          this.loadBookings();
          Swal.fire({ icon: 'success', title: 'Cancelled', text: 'Your booking has been cancelled.', timer: 2000, showConfirmButton: false });
        },
        error: () => {
          this.isCancelling = false;
          this.errorMessage = 'Failed to cancel booking.';
          Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to cancel booking. Please try again.', confirmButtonColor: '#ef4444' });
        }
      });
    });
  }
