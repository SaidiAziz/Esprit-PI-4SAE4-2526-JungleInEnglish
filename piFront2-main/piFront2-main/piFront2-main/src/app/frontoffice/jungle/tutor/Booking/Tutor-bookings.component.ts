...
export class TutorBookingsComponent implements OnInit {
  ...
  constructor(
    ...
    private appNotif: AppNotificationService,
  ...
  confirmBooking(booking: TutorBooking): void {
    ...
    this.bookingService.confirm(booking.id).subscribe({
      next: () => {
        ...
        this.appNotif.success('Réservation acceptée', 'Lien de paiement envoyé à l\'étudiant.');
        Swal.fire({
          icon: 'success',
  ...
  private sendPaymentRequestEmail(booking: TutorBooking): void {
    ...
    this.emailService.sendEmail(emailParams).catch((err: unknown) => console.error('Erreur envoi email:', err));
  }
  ...
  confirmReject(): void {
    ...
    this.bookingService.reject(this.selectedBooking.id, reason).subscribe({
      next: () => {
        ...
        this.loadBookings();
        this.appNotif.warning('Réservation rejetée', reason);
      },
  ...
  cancelBooking(id: number | undefined): void {
    ...
    Swal.fire({
      ...
      this.bookingService.cancel(id, 'TUTOR', 'Cancelled by tutor').subscribe(() => {
        this.loadBookings();
        this.appNotif.info('Réservation annulée', 'La réservation a été annulée.');
      });
    });
  }
