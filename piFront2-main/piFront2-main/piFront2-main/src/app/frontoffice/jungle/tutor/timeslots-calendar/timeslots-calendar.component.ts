...
export class TimeslotsCalendarComponent implements OnInit {
  ...
  openEdit(event: MouseEvent, e: TimeSlot) {
    ...
    this.editingId = (typeof e.id === 'number') ? e.id : null;
    this.form = {
  ...
  save() {
    ...
    const payload = {
      ...
      // ✅ FORCE "HH:mm:ss"
      startTime: this.toHHMMSS(this.form.startTime),
      endTime: this.toHHMMSS(this.form.endTime),
    };
    ...
    if (this.editingId !== null) {
      this.api.updateTimeslot(this.editingId, payload).subscribe({
        next: () => { this.saving = false; this.closeModal(); this.loadTimeslots(); },
        error: (err: any) => {
