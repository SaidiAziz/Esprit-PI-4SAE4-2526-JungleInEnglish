...
- **Backend API:** `http://localhost:8222/auth`
...
**API base:** `http://localhost:8222/auth`
...
3. **HTTP** — `authService.login({ email, password })` → `POST http://localhost:8222/auth/login`
...
`SignupComponent` calls `authService.register()` → `POST http://localhost:8222/auth/register`.
