...
public class TutorPServiceImpl implements TutorPServiceI{
    ...
    public TutorProfile createTutorProfile(TutorProfile tutorProfile) {
        if (tutorProfile.getUser() != null && tutorProfile.getUser().getId() != null) {
            TutorProfile existingProfile = tutorProfileRepository.findByUser_Id(tutorProfile.getUser().getId()).orElse(null);
