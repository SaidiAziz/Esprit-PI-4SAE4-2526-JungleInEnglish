...
import { Session, SessionPayload } from '../models/session.model';
...
export class SessionService {
  ...
  create(payload: SessionPayload): Observable<Session> {
    return this.http.post<Session>(this.baseUrl, payload);
  }

  update(id: number, payload: SessionPayload): Observable<Session> {
  ...
  extractBackendMessage(err: any): string {
    return (
      err?.error?.details ||
      err?.error?.message ||
      err?.error?.error ||
